...
public class DhmsMontrealImplementation implements DhmsMontreal {
    ...
    public String addAppointment(String appointment_id, String type, int capacity) {
        ...
        if(flag.equals(1)){
            ...
            try {
                File log_file = new File("D:\\web_services\\montreal_server\\logs\\admin_log.txt");
                if (log_file.createNewFile()) {
                    FileWriter log_file_writer = new FileWriter("D:\\web_services\\montreal_server\\logs\\admin_log.txt");
                    ...
                    BufferedWriter log_file_buffered_writer = new BufferedWriter(new FileWriter("D:\\web_services\\montreal_server\\logs\\admin_log.txt", true));
    ...
    public String bookAppointment(String patient_id, String appointment_id, String type) {
        ...
        if(count >= 3){
            ...
            if(flag.equals(1)){
                ...
                //creating log file and keeping records
                try {
                    File log_file = new File("D:\\web_services\\montreal_server\\logs\\"+ patient_id +".txt");
                    if (log_file.createNewFile()) {
                        //client file writer
    ...
    public String listAppointmentAvailability(String appointment_type) {
        ...
        database db = new database();
    	int array_size = db.appointments.get(appointment_type).values().size() + records_quebec.length + records_sherbrooke.length;
